...
import math
from app.models.schemas import BallparkStatsSchema, PitcherStatsSchema, TeamStatsSchema
...
class NRFIModel:
    ...
    def _get_pitcher_stats(self, pitcher_name: str) -> tuple[float, float]:
        ...
        validated = PitcherStatsSchema.model_validate(data)
        fip = validated.fip
    ...
    def _get_team_stats(self, team_name: str) -> TeamStats:
        ...
        validated = TeamStatsSchema.model_validate(
            {
                "wrc_plus": advanced.get("wrc_plus", 100.0),
                "off_current": offense.get("current", 4.5),
                "off_last3": offense.get("last_3", 4.5),
                "def_current": defense.get("current", 4.5),
            }
        )
        return TeamStats(
            ...
            wrc=validated.wrc_plus,
        )

    def _get_ballpark_stats(self, home_team: str) -> tuple[float, float]:
        park_data = self.ballpark_db.get(home_team, {})
        validated = BallparkStatsSchema.model_validate(park_data)
        return validated.nrfi_pct, validated.yrfi_pct
    ...
    def calculate(
        ...
        nrfi_score = (
            0.25 * pNRFIa
            + 0.25 * pNRFIh
            + 0.05 * away_stats.t_nrfi
            + 0.05 * home_stats.t_nrfi
            + 0.15 * parkN
            + 0.03 * (1.0 - baThrA)
            + 0.03 * (1.0 - baThrH)
            + 0.03 * (1.0 - wrcThrA)
