    ...
    def fetch_todays_weather(self, matchups_data: dict) -> dict:
        ...
        for game in games:
            ...
            if not park_info or "lat" not in park_info:
                print(
                    f"⚠️ {home_team} stadyum koordinatları bulunamadı, varsayılan atanıyor."
                )
            ...
            try:
                ...
                if roof_type == "Retractable" and (
                    "Rain" in condition
                    or "Snow" in condition
                    or "Thunderstorm" in condition
                ):
                ...
                print(
                    f"❌ {home_team} Hava Durumu Hatası: {e}. Varsayılan değer atanıyor."
                )
        ...
        print(
            f"🎉 Toplam {len(weather_data)} stadyumun hava durumu başarıyla güncellendi."
        )
        return weather_data

    async def fetch_todays_weather_async(
        self, client: httpx.AsyncClient, matchups_data: dict
    ) -> dict:
        print("☁️ Stadyumların canlı hava durumu verileri çekiliyor...")
