...
import httpx
from app.services.mlb_unified_engine import MLBUnifiedEngine, GameInputData
...
class PredictionRunner:
    ...
    async def _run_scrapers_async(self, client: httpx.AsyncClient) -> bool:
        ...
        print(
            "🎯 [3/5] Gelişmiş Atıcı istatistikleri çekiliyor (MLB API - Statcast)..."
        )
        ...
        results = await asyncio.gather(
            pitcher_task, odds_task, weather_task, return_exceptions=True
        )

        if isinstance(results[0], Exception):
            print(
                f"⚠️ Uyarı: PitcherScraper hatası. Lig ortalamaları kullanılacak. ({results[0]})"
            )
        if isinstance(results[1], Exception):
            print(
                f"⚠️ Uyarı: OddsProvider hatası. Oran karşılaştırması atlanacak. ({results[1]})"
            )
        if isinstance(results[2], Exception):
            print(
                f"⚠️ Uyarı: WeatherScraper hatası. Standart hava atandı. ({results[2]})"
            )
        ...
        for game_dict in games:
            try:
                try:
                    ...
                    print(
                        f"❌ Veri Formatı Hatası ({game_dict.get('away_team')} vs {game_dict.get('home_team')}): {ve}"
                    )
                    continue
