...
CACHE_EXPIRY_SECONDS = settings.CACHE_EXPIRY_SECONDS  # 1 Saat
...
async def get_predictions():
    ...
    if need_refresh:
        ...
        async with _update_lock:
            ...
            if (
                ...
                and (time.time() - os.path.getmtime(PREDICTIONS_FILE))
                < CACHE_EXPIRY_SECONDS
            ):
                need_refresh = False
            else:
                print(
                    "🔄 Veriler bayatlamış (veya yok), motor izole havuzda (threadpool) çalıştırılıyor..."
                )
    ...
    return {"status": "success", "cached": not need_refresh, "data": data}
...
async def refresh_data():
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Tüm sistem verileri başarıyla zorla güncellendi.",
        }
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Manuel güncelleme hatası: {str(e)}"
        )
